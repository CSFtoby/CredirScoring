...
namespace Docsis_Application
{
	public partial class MDI_Menu : Form
	{
		...
		//public static string usuario_wstransunion = "sagrada_familia_test";
		//public static string pass_wstransunion = "Test123!!!";
		...
		private void MDI_Menu_Load(object sender, EventArgs e)
		{
			...
			//    btnConfigurarWf.Enabled = true;
			//else
			//    btnConfigurarWf.Enabled = false;
		...
		private void p_cambiar_modo(string pa_operacion)
		{
			if (pa_operacion == "CONS")
			{
				...
				panel_indicativo_superior.BackColor = Color.FromArgb(252, 167, 66);
			...
			{
				DocSys.vl_operacion = "NORM";
				panel_indicativo_superior.BackColor = Color.FromArgb(255, 255, 220);
				txtTexto_buscar.Text = "Ingrese texto a buscar";
		...
		private void p_set_grid_attr_visual()
		{
			...
			for (int row = 0; row < gv_entradas.Rows.Count; row++)
			{
				...
				/*string vl_nodo_selected = "";
                try
                {
                    vl_nodo_selected = treeView.SelectedNode.Name.ToString().ToUpper();
                }
                catch
                {
                    vl_nodo_selected = "";
                ...
                if (vl_nodo_selected != "NODE_ENTRADAS")
                {
                    MessageBox.Show("");
                }*/
		...
		private void p_llenar_combo_workflows()
		{
			try
			{
				...
				//ComboBox_wokflows.ComboBox.DataSource = dsCombo;
				//ComboBox_wokflows.ComboBox.DisplayMember = "dcs_workflows.nombre_workflow";
				//ComboBox_wokflows.ComboBox.ValueMember = "dcs_workflows.workflow_id";
				//ComboBox_wokflows_SelectedIndexChanged(null, null);
		...
		*/
		...
		private void gv_Entradas_Sorted(object sender, EventArgs e)
		{
			...
			//gv_Entradas_SelectionChanged(null, null);
		}
		...
		private void contextMenuStrip1_Opening(object sender, CancelEventArgs e)
		{
			...
			//if (global_estacion_id_to == u_Globales.eAfiliacion /*&& ver_Excepciones*/ && codigoZona != 2)
			//{
			...
			//	nuevaExcepcionToolStripMenuItem.Visible = false;
		...
		{
			//int vl_solic_entrada = p_get_cant_solic_entradas(DocSys.vl_workflow_id);
			//int vl_solic_denegadas = p_get_cant_solic_denegadas(DocSys.vl_workflow_id);
			...
			//nodehijo = node.Nodes.Add("Solicitudes para cerrar");
			//nodehijo.ImageKey = "icon_collapse_tree.png";
			//nodehijo.SelectedImageKey = "icon_treeclosed.png";
			//nodehijo.Name = "Node_porcerrar";
		...
		private void comboBoxEstacionUsuario_SelectionChangeCommitted(object sender, EventArgs e)
		{
			...
			//Si es Prestamos TGU,SPS,LCBA activa la funcion
			if (global_estacion_id_to == 1003 || global_estacion_id_to == 2002 || global_estacion_id_to == 3002)
			{
				asignarLaSolicitudToolStripMenuItem.Enabled = true;
				observacionesDePrestamosToolStripMenuItem.Enabled = true;
			}
			else
			{
				asignarLaSolicitudToolStripMenuItem.Enabled = false;
				observacionesDePrestamosToolStripMenuItem.Enabled = false;
			}
			...
			treeView.ExpandAll();
			string ver_Excepciones = DocSys.puede_ver_excepciones(global_estacion_id_to);
			if (!ver_Excepciones.Equals(string.Empty) /*& this.dgvExcepciones.Visible*/)
			{
		...
		{
			//panelCargandoSolic.Location = new Point(this.ClientSize.Width / 2 - panelCargandoSolic.Size.Width / 2, this.ClientSize.Height / 2 - panelCargandoSolic.Size.Height / 2);
			//panelCargandoSolic.Location = new Point((this.ClientSize.Width / 2 - panelCargandoSolic.Size.Width / 2) - 200,  panelCargandoSolic.Location.Y);
			//panelCargandoSolic.Location = new Point((this.ClientSize.Width / 2 - panelCargandoSolic.Size.Width / 2) - 200, gv_entradas.Location.Y+100);
		...
		{
			//s_Conciliacion_doc forma = new s_Conciliacion_doc();
		...
		private void enviar_excepcion_Click(object sender, EventArgs e)
		{
			if (this.dgvExcepciones.RowCount > 0)
			{
				...
				{
					MessageBox.Show("La solicitud que esta tratando de enviar ya fue procesada por otro usuario !", DocSys.vl_mensaje_avisos, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
					//p_refrescar_info(); //modificar para la de excepciones
					return;
				}
		...
		private void p_set_grid_attr_visual_Exc()
		{
			for (int row = 0; row < this.dgvExcepciones.Rows.Count; row++)
			{
				...
				/*else
				{
					...
					style.Font = new Font(dgvExcepciones.Font, FontStyle.Regular);
					dgvExcepciones.Rows[row].DefaultCellStyle = style;
		...
		{
			// para que no se popule el trevieww
			//p_popular_treview();
		...
		{
			if (global_estacion_id_to == 1000 | global_estacion_id_to == 1001)
			{
