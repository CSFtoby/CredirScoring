...
namespace Docsis_Application
{
    public partial class s_analisis_cuantitativo : Form
    {
        ...
        private void llenar_analisis_cuanti()
        {
            ...
            {
                /*Para quitar que el analisis cuantitativo sea del aprobado*/

                //string vl_estado_id = dtCuanti.Rows[0]["decision_final_solicitud"].ToString();
                //if (vl_estado_id!="APROBADO")
                //{
                //    MessageBox.Show("La solicitud debe estar en estado APROBADO para poder emitar el Analisis Cuantitativo", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                //    this.Close();
                //    return;
                //}
                //else
                ...
                try
                {
                   ...
                   // txtSeguro_vida.Text = string.Format("{0:#,###,###,##0.00}", float.Parse(dtCuanti.Rows[0]["seguro_vida"].ToString()));
                   // txtSeguro_danos.Text = string.Format("{0:#,###,###,##0.00}", float.Parse(dtCuanti.Rows[0]["seguro_danos"].ToString()));
                   // txtSeguro_incendio.Text = string.Format("{0:#,###,###,##0.00}", float.Parse(dtCuanti.Rows[0]["seguro_incendio"].ToString()));
                   // txtSeguro_vida_mens.Text = string.Format("{0:#,###,###,##0.00}", float.Parse(dtCuanti.Rows[0]["seguro_vida_mensual"].ToString()));
                   // txtSeguro_incendios_mens.Text = string.Format("{0:#,###,###,##0.00}", float.Parse(dtCuanti.Rows[0]["seguro_incendios_mensual"].ToString()));
                   // txtSeguro_danos_mens.Text = string.Format("{0:#,###,###,##0.00}", float.Parse(dtCuanti.Rows[0]["seguro_danos_mensual"].ToString()));
                ...
                try
                {
                    ...
                    //txtTasa.Text = string.Format("{0:##0.00}", float.Parse(dtCuanti.Rows[0]["tasa_sol"].ToString()));
                    //decimal tasa_decimal = Convert.ToDecimal(dtCuanti.Rows[0]["tasa_sol"].ToString(), System.Globalization.CultureInfo.GetCultureInfo("no"));
                    decimal tasa_decimal = Convert.ToDecimal(dtCuanti.Rows[0]["tasa_sol"].ToString());
                    txtTasa.Text = tasa_decimal.ToString();
                }
                catch
                {
                ...
                txtCuota_nivelada.Text = da.ObtenerCuotaNivela(vl_codigo_sub_app,float.Parse(txtMonto_solicitado.Text), float.Parse(txtTasa.Text), float.Parse(txtPlazo.Text)).ToString();                
                txtEdad.Text = dtCuanti.Rows[0]["edad"].ToString();
                ...
                /*DataTable dtDeducciones_mcr = new DataTable();
                dtDeducciones_mcr.Columns.Add("cod_tipocobro");
                dtDeducciones_mcr.Columns.Add("des_tipocobro");
                dtDeducciones_mcr.Columns.Add("mon_cobro");
                dtDeducciones_mcr.Columns.Add("mon_cobro_original");
                dtDeducciones_mcr.Columns.Add("cod_moneda_cobro");                
                string xml_deducciones_mcr = txtXmlDeduccionesMCR.Text;*/

                try
                {
                    //DataTable dt = xml_to_datatable(xml_deducciones_mcr);
                    //gvGastos_desembolso.Rows.Clear();

                    /*for (int x = 0; x < dt.Rows.Count; x++)
                    {
                        dtDeducciones_mcr.Rows.Add(dt.Rows[x][0].ToString(),
                                              dt.Rows[x][1].ToString(),
                                              decimal.Parse(dt.Rows[x][2].ToString()).ToString("###,###,###,##0.00"),
                                              decimal.Parse(dt.Rows[x][3].ToString()).ToString("###,###,###,##0.00"),
                                              dt.Rows[x][4].ToString());
                    }*/
                    //gvGastos_desembolso.DataSource = dtDeducciones_mcr;
