	...
	public partial class e_nueva_excepcion_solicitud : Form
	{
		...
		private bool advertencia_agregar()
		{
			...
			/*foreach (DataGridViewRow item in this.dgvExcepciones.Rows)
            {
                ...
                {
                    string rowLin = item.Cells["cod_lineamiento"].Value.ToString();
                    string rowExc = item.Cells[2].Value.ToString();

                    if (rowLin.Equals(this.v_CodLineamiento) && rowExc.Equals(this.v_CodigoExcepcion))
                    {
                        this.lblError.Text = "La excepción ya fue seleccionada";
                        MessageBox.Show("La excepción ya fue añadida.");
                        return true;
                    }

                    //evalua vrs lineamiento

                  
                    //lineamiento del arreglo con excepcion seleccionada
                    int prohLE = reglas_excepciones.Where(c => c.codigo_lineamiento.Equals(rowLin)
                                                            && c.cod_tipo_excep_prohibida.Equals(this.v_CodigoExcepcion)).ToList().Count;

                    //excepcion del arreglo con excepcion seleccionada
                    int prohEE = reglas_excepciones.Where(c => c.cod_tipo_excepcion.Equals(rowExc)
                                                            && c.cod_tipo_excep_prohibida.Equals(this.v_CodigoExcepcion)).ToList().Count;

                    
                    //excepcion del arreglo con lineamiento seleccionado
                    int prohEL = reglas_excepciones.Where(c => c.cod_tipo_excepcion.Equals(rowExc)
                                                            && c.cod_linea_prohibida.Equals(this.v_CodLineamiento)).ToList().Count;

                    //lineamiento del arreglo con lineamiento seleccionado
                    int prohLL = reglas_excepciones.Where(c => c.codigo_lineamiento.Equals(rowLin)
                                                            && c.cod_linea_prohibida.Equals(this.v_CodLineamiento)).ToList().Count;

                    if (prohLE > 0)
                    {
                        foreach (var p in reglas_excepciones.Where(c => c.codigo_lineamiento.Equals(rowLin)
                                                          && c.cod_tipo_excep_prohibida.Equals(this.v_CodigoExcepcion)).ToList())
                        {
                            if (p.observaciones.Equals(string.Empty))
                            {
                                mensaje += $"*No se puede combinar {p.codigo_lineamiento} con {p.cod_tipo_excep_prohibida}\n";
                            }
                            else
                            {
                                mensaje += $"*{p.observaciones}";
                            }
                        }
                    }
                    ...
                    if (prohEE > 0)
                    {
                        foreach (var p in reglas_excepciones.Where(c => c.cod_tipo_excepcion.Equals(rowExc)
                                                          && c.cod_tipo_excep_prohibida.Equals(this.v_CodigoExcepcion)).ToList())
                        {
                            if (p.observaciones.Equals(string.Empty))
                            {
                                mensaje += $"*No se puede combinar {p.cod_tipo_excepcion} con {p.cod_tipo_excep_prohibida}";
                    ...
                    if (prohLE > 0 | prohEE > 0 | prohEL > 0 | prohLL > 0)
                    {
                        detener = true;
                        this.lblRegla.Visible = true;
                        this.lblRegla.Text = mensaje;
                        this.lblError.Text = "Hay una o más combinaciones de excepción no permitidas. Revise las opciones seleccionadas";
                    }
                    else
                    {
                        detener = false;
