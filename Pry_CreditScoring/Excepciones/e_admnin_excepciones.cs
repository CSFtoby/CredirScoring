...
namespace Docsis_Application.Excepciones
{
	public partial class e_admnin_excepciones : Form
	{
        ...
        private void btn_quita_M_Click(object sender, EventArgs e)
        {
            if (this.dgvResoluciones.Rows.Count > 0)
            {
                DataGridViewRow row = this.dgvResoluciones.CurrentRow;
                ...
                int excp = Convert.ToInt32(txtExcepcion.Text);
                int numero_movimiento = 0;
                int registro = 0;

                DataTable dt = da.get_detalle_excp(excp, vl_usuario);
                if (dt != null) {
                    foreach (DataRow dr in dt.Rows) {
                        numero_movimiento = Convert.ToInt32(dr["no_movimiento"].ToString());
                        registro = Convert.ToInt32(dr["registro_ap"].ToString());
                    }
                }
                ...
                if (String.IsNullOrEmpty(vl_comentario))
                {
                    if (pendiente.Equals("S"))
                    {
                        if (MessageBox.Show("Desea eliminar el registro del usuario " + vl_usuario + " ?", "Aviso de Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            if (Convert.ToBoolean(DialogResult.Yes) == true)
                            {
                                if (da.EliminarExcepcionxUsuario(excp, vl_usuario, numero_movimiento, registro) == true)
                                {
                    ...
                    {
                        MessageBox.Show("No se puede quitar porque ya fue resuelta.");
                        return;
                    }
                }
                else {
                    MessageBox.Show("Para quitar el miembro debera prinero quitar comentarios.");
                    return;
                }
            }
        }

        private void btn_quita_C_Click(object sender, EventArgs e)
        {
            if (this.dgvResoluciones.Rows.Count > 0)
            {
                DataGridViewRow row = this.dgvResoluciones.CurrentRow;
                ...
                else
                {
                    if (MessageBox.Show("Desea eliminar el Comentario del usuario " + vl_usuario + " ?", "Aviso de Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        if (Convert.ToBoolean(DialogResult.Yes) == true)
                        {
                            if (da.EliminarComentarioxUsuarioEXCP(excp, vl_usuario, numero_movimiento, registro) == true)
                            {
                                MessageBox.Show("Se ha quitado con exito");
                                cargar_generales();
                            }
