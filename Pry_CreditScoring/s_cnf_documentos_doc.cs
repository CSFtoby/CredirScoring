    ...
    public partial class s_cnf_documentos_doc : Form
    {
        ...
        private void s_cnf_documentos_doc_Load(object sender, EventArgs e)
        {
            ...
            if (vl_accion == "ELIM")
            {
                ...
                rbPersona5.Enabled = false;
                rbPersona6.Enabled = false;
                rbPersona7.Enabled = false;
                button1.Focus();
        ...
        private void p_datos_documento()
        {
            try
            {
                ...
                if (dr.HasRows)
                {
                    ...
                    if (vl_persona == "P5")
                        rbPersona5.Checked = true;
                    if (vl_persona == "P6")
                        rbPersona6.Checked = true;
                    if (vl_persona == "P7")
                        rbPersona7.Checked = true;
                }
                dr.Close();
            }
            catch (Exception ex)
        ...
        private void p_abm_documentos()
        {
            ...
            if (rbPersona4.Checked)
                vl_persona = "P4";
            if (rbPersona5.Checked)
                vl_persona = "P5";
            if (rbPersona6.Checked)
                vl_persona = "P6";
            if (rbPersona7.Checked)
                vl_persona = "P7";
            try
            {
                if (DocSys.connOracle.State.ToString().ToUpper() == "CLOSED")
