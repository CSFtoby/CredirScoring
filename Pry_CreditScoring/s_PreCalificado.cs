...
namespace Docsis_Application
{
    public partial class s_PreCalificado : Form
    {
        ...
        private void calcular_cuotas_consolo(int indice)
        {
            ...
            for (int i = 0; i < gvCuotasBuro.Rows.Count; i++)
            {
                ...
                if (seleccionada == "true")
                {
                    ...
                    total_descuento = Convert.ToDouble(txtDeudas_desc_sol.Text);
                    total_capital = total_capital + double.Parse(saldos);
                }
            }
            total_pago = (total_capital - total_descuento);
            txtTotal_cuotas_consolidar.Text = total_cuotas.ToString("###,###,##0.00");
            txtTotal_capital_consolidar.Text = total_pago.ToString("###,###,###,##0.00");
        }
                ...
                if (gmodo_coopsafa.Equals("INS"))
                {
                    ...
                    //{
                        ...
                        else
                        {
                            ...
                            Identidad_Delegado = dt.Rows[0]["numero_identificacion"].ToString();
        ...
        private void p_get_tipo_Miembro(string p_no_identificacion)
        {
            DataTable dt = da.ObtenerTipoMiembroXID(p_no_identificacion);
            this.AdvertenciaGeneral = string.Empty;

            if (dt.Rows.Count > 0)
            {
                if (dt.Rows[0]["tipo_miembro"].ToString() == string.Empty)
                {
                    txtMiembro.Visible = false;
                    lbMiembro.Visible = false;
                }
                else
                {
                    txtMiembro.Text = dt.Rows[0]["tipo_miembro"].ToString();
                    txtMiembro.BackColor = Color.FromArgb(111, 255, 51);
                    if (txtMiembro.Text == "NO ES DELEGADO")
                    {
                        txtMiembro.Visible = false;
                        lbMiembro.Visible = false;
                    }
        ...
        private void p_llenar_combo_destinos()
        {
            try
            {
                //FELVIR01 - Para el filtro de destino. 18/08/2020
                if (!string.IsNullOrEmpty(this.txtCodigo_sub_aplicacion.Text))
                {
                    int tipoPrestamo = Convert.ToInt32(this.txtCodigo_sub_aplicacion.Text);
                    DataTable dtDestinos = da.ObtenerDestinoCredito(tipoPrestamo);
        ...
        //revisión de parte para modificación segun nuevos requerimientos 
        private string p_construir_xml(string p_tipo)
        {
            ...
            //parte donde se agrega el descuento del solicitante
            #region
            string porcion_xml_desc_sol = "";
            string descuento_solicitante = "0";
            try
            {
                descuento_solicitante = string.Format(String.Format("{0:########0.00}", double.Parse(this.txtDeudas_desc_sol.Text)));
            }
            catch
            {
            ...
            //if (txtDeudas_desc_sol.Text == "0.00")
            //{
            //    porcion_xml_desc_sol = @"<MontoDescuento/>";
            //}
            //else
            //{
            //    porcion_xml_desc_sol = @"<MontoDescuento>" + descuento_solicitante + @"</MontoDescuento>";
            //}
            ...
            xml_solicitante = @"<Multi>
                                     ...
                                     <MontoDescuento>" + descuento_solicitante + @"</MontoDescuento>
            ...
            //<MontoDescuento></MontoDescuento> para integrarlo a xml porcion_xml_desc_sol +
        ...
        private void procesar_xml_respuesta(string p_resultado_consulta, string p_accion, string p_modio_ori_transunion)
        {
            ...
            try
            {
                ...
                if (vl_Status.ToString() == "Success")
                {
                    ...
                    {
                        throw new Exception("Error al leer el Score del afiliado. Verifique si el afiliado se encuentra en COOPSAFA II y III. Caso contrario, contacte al administrador");
                    }
        ...
        private void btnGuardar_solicitud_Click(object sender, EventArgs e)
        {
            ...
            if (gmodo_coopsafa == "INS")
            {
                if (MessageBox.Show("Desea guardar la solicitud siguiente :            " + vl_enter + vl_enter +
                                ...
                                " Plazo " + Convert.ToDecimal(txtPlazo.Text) + vl_enter +
            ...
            decimal vl_tasa = 0;
            decimal.TryParse(txtTasa.Text, out vl_tasa);
            sol.tasa = vl_tasa;
            ...
            decimal descuento_solicitante = decimal.Parse(this.txtDeudas_desc_sol.Text);

            if (descuento_solicitante == 0.0m)
            {
                sol.DeudasDescSol = 0;
            }
            else
            {
                sol.DeudasDescSol = descuento_solicitante;
            }
            ...
            if (gmodo_coopsafa == "INS")
            {
                ...
                if (da.InsertarSolicitudCredito(sol, out no_solicitud, gar))
                {
                    ...
                    if (sol.codigo_sub_aplicacion == 19 & sol.patrono.Contains("MINISTERIO PUBLICO"))
                    {
                        ...
                        {
                            var prgtiempo = MessageBox.Show("¿El credito solicitado corresponde al año siguiente?", "Imprimir Autorización", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
                            var anio_siguiente = false;
                            if (prgtiempo == DialogResult.Yes) {
                                anio_siguiente = true;
                            }
                            DocMinisterioPublico reporte = new DocMinisterioPublico(sol.no_solicitud, this.da, Convert.ToInt32(gcodigo_agencia), anio_siguiente);
                            reporte.ShowDialog();
                        }
                    }
                    return;
        ...
        private void lLPonderacion_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            if (vl_monto_soli > 0)
            {
                ...
                {
                    int tipoPrestamo = Convert.ToInt32(this.txtCodigo_sub_aplicacion.Text);
                    s_PreCalificado_info05 forma = new s_PreCalificado_info05(tipoPrestamo);
        ...
        private void txtIDSolicitante_TextChanged(object sender, EventArgs e) { }

        private void txtDeudas_desc_sol_Leave(object sender, EventArgs e)
        {
            TextBox texbox = txtDeudas_desc_sol;
            double valor_ingresado = 0;
            if (!string.IsNullOrEmpty(texbox.Text))
                valor_ingresado = Convert.ToDouble(texbox.Text);
        ...
        private void txtCodigo_sub_aplicacion_TextChanged(object sender, EventArgs e)
        {
            int tipoPrestamo = 0;
            bool isNum = Int32.TryParse(this.txtCodigo_sub_aplicacion.Text, out tipoPrestamo);
