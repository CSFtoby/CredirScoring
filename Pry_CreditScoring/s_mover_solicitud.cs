...
namespace Docsis_Application
{
    public partial class s_mover_solicitud : Form
    {
        ...
        private void btnMoverSolicitud_Click(object sender, EventArgs e)
        {
            ...
            DataTable deH = DocSys.LlenarHorario();
            DataTable HOR = DocSys.ObtenerHoras();

            string day = Convert.ToString(today.DayOfWeek);
            bool coincidencia = DocSys.CoincideDia(day);
               
            if (labelDestino.Text.Equals("Creditos TGU") || labelDestino.Text.Equals("Creditos SPS") || labelDestino.Text.Equals("Creditos LCBA"))
            {
                if (coincidencia == true)
                {
                    //if (Convert.ToInt32(HOR.Rows[0]["HORA"].ToString()) >= Convert.ToInt32(deH.Rows[0]["HORA_INICIAL"].ToString()) && 
                    //    Convert.ToInt32(HOR.Rows[0]["MIN"].ToString()) >= Convert.ToInt32(deH.Rows[0]["MIN_INICIAL"].ToString()))
                    //{
                        //if (Convert.ToInt32(HOR.Rows[0]["HORA"].ToString()) < Convert.ToInt32(deH.Rows[0]["HORA_FINAL"].ToString()))
                        //{
                            ...
                            //&& cant_resoluciones_selec != resol_req valida si esta ccorrecto
                            ...
                            ///---------cambio para poder enviar las solicitudes sin error, validar si esta correcto 
                        ...
                        //else
                        //{
                        //    MessageBox.Show("¡¡el horario de envio de solicitudes es de " + Convert.ToInt32(deH.Rows[0]["HORA_INICIAL"].ToString()) + " a " + Convert.ToInt32(deH.Rows[0]["HORA_FINAL"].ToString()));
                        //}
                    //}
                    //else
                    //{
            ...
            if (labelDecisionID.Text != "" & labelPaso.Text != "")
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (es_nivel_resolutivo == "S")
                    {
                        ...
                        //Filtro de ventanilla
                        string vl_forma_pago = da.ObtenerEsVentanillaPlanilla(gno_solicitud);
                        //añadir pago por planilla, apribado hasta 300,000. Referido o rechazado hasata 200,000
                        if (labelDestino.Text == "Nivel Resolutivo Filial" && vl_forma_pago == "PLANILLA" && monto_sol > 800000)
                        {
                            btnMoverSolicitud.Enabled = false;
                            labelAvisos.Text = "Este nivel no puedo ver solicitudes de afiliados por ventanilla por montos mayores a L100,000.00";
                        }

                        if (labelDestino.Text == "Nivel Resolutivo Filial" && vl_forma_pago == "VENTANILLA" && monto_sol > 800000)
                        {
                            btnMoverSolicitud.Enabled = false;
                            labelAvisos.Text = "Este nivel no puedo ver solicitudes de afiliados por ventanilla por montos mayores a L100,000.00";
                        }
